...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_GATEWAY_URL}/pim/api/v1/marketplace/attributes${queryString ? `?${queryString}` : ''}`;
    ...
    console.log('✅ Proxy API GET Response:', { 
      ...
      count: Array.isArray(data.attributes) ? data.attributes.length : data.length,
      structure: data.attributes ? 'attributes_array' : 'unknown',
      attributes_received: data.attributes?.length || 0
    });
